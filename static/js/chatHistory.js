async function initializeChatHistory() {
    try {
        ...
        document.getElementById("chatHistory").innerHTML = "<p>Error loading user information. Please try again later.</p>";
    }
}
...
async function displayChatHistory(userId) {
    ...
    try {
        const response = await fetch(`http://localhost:8000/api/get_chat_history/${userId}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
        });
        ...
        document.getElementById("chatHistory").innerHTML = "<p>Error loading chat history. Please try again later.</p>";
    }
}
