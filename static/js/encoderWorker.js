...
OggOpusEncoder.prototype.getChecksum = function (data) {
  ...
  for (var i = 0; i < data.length; i++) {
    checksum =
      (checksum << 8) ^
      this.checksumTable[((checksum >>> 24) & 0xff) ^ data[i]];
  }
  return checksum >>> 0;
};
...
OggOpusEncoder.prototype.generatePage = function () {
  ...
  var pageBuffer = new ArrayBuffer(
  ...
  if (granulePosition < 0) {
    pageBufferView.setInt32(
      10,
      Math.ceil(granulePosition / 4294967297) - 1,
      true,
    );
  } else {
...
OggOpusEncoder.prototype.initCodec = function () {
  ...
  this.encoder = this._opus_encoder_create(
    this.config.encoderSampleRate,
    this.config.numberOfChannels,
    this.config.encoderApplication,
    errLocation,
  );
  ...
  this.encoderSamplesPerChannel =
    (this.config.encoderSampleRate * this.config.encoderFrameSize) / 1000;
  ...
  this.encoderBufferLength =
    this.encoderSamplesPerChannel * this.config.numberOfChannels;
  this.encoderBufferPointer = this._malloc(this.encoderBufferLength * 4); // 4 bytes per sample
  this.encoderBuffer = this.HEAPF32.subarray(
...
OggOpusEncoder.prototype.initResampler = function () {
  ...
  this.resampler = this._speex_resampler_init(
    this.config.numberOfChannels,
    this.config.originalSampleRate,
    this.config.encoderSampleRate,
  ...
  this.resampleSamplesPerChannel =
    (this.config.originalSampleRate * this.config.encoderFrameSize) / 1000;
  this.resampleSamplesPerChannelPointer = this._malloc(4);
  this.HEAP32[this.resampleSamplesPerChannelPointer >> 2] =
    this.resampleSamplesPerChannel;

  this.resampleBufferLength =
    this.resampleSamplesPerChannel * this.config.numberOfChannels;
  this.resampleBufferPointer = this._malloc(this.resampleBufferLength * 4); // 4 bytes per sample
  this.resampleBuffer = this.HEAPF32.subarray(
        ...
        if (this.encoder) {
          switch (data["command"]) {
            ...
            case "done":
              this.encoder
                .encodeFinalFrame()
                .forEach((pageData) => this.postPage(pageData));
